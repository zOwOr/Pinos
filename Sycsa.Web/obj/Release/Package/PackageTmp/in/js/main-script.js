...
//    var a = Number(a);
//    var b = Number(b);
//    return a > b ? 1 : a < b ? -1 : 0; // for newer on top
//    //return a < b ? -1 : a == b ? 0 : 1; // for older on top
//}
...
include('in/js/tinynav.min.js'); // Tiny nav
...
$(document).ready(function(){
   ...
   //             dropdown.css('opacity', 0).css('top', '150%').show();//.animate({opacity:1, top:100+'%'},300);
            ...
            if (dropdown && dropdown.length > 0 && !dropdown.classList.contains('animating') && windowWidth > 767) {
                dropdown.style.opacity = '0';
                dropdown.style.top = '150%';
                dropdown.style.display = 'block';

                // Agregar una clase 'animating' para evitar que se active varias veces
                dropdown.classList.add('animating');

                // Hacer la animación de fade-in y mover hacia arriba
                setTimeout(function () {
                    dropdown.style.opacity = '1';
                    dropdown.style.top = '100%';
                }, 0); // Esto coloca la animación en la cola de tareas pendientes para que se ejecute después de que se apliquen los estilos anteriores
            }
            ...
            if (dropdown && dropdown.length > 0 && !dropdown.classList.contains('animating')) {
                dropdown.classList.add('animating');

                // Hacer la animación de fade-out y mover hacia abajo
                dropdown.style.opacity = '0';
                dropdown.style.top = '150%';
                dropdown.style.display = 'block';
	...
	function searchBar(){
		...
		searchButton.click(function(){
			...
			if(searchBar.hasClass('searchbar-visible')){
				...
				searchBar.css('opacity', 0).css('left', -200).css('right',200).show().animate({opacity:1, left:0, right:120},300, function(){
	...
	function backToTop(){
        ...
        window.addEventListener('scroll', function () {
            ...
            var w_scroll = window.pageYOffset || document.documentElement.scrollTop;

            if (w_scroll > 150 && window.innerWidth > 767) {
    ...
    function scrollToTop(duration) {
        ...
        var animateScroll = function () {
            ...
            var scrollTopVal = Math.easeInOutQuad(currentTime, start, -start, duration);
