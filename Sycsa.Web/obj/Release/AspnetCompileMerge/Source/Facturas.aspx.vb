...
Public Class Facturas
    ...
    Private Sub BindListView()
        ...
        DataSet = common.sqlconsulta("SELECT [CLAVE] ,[EMPRESA] ,[TIPO_DOC] ,[FOLIO] ,[CLIENTE] ,[FECHA_ALTA] ,[FECHA_BAJA] ,[ORDEN_COMP] ,[LISTA] ,[DESC_VOL] ,[SUB_TOTAL] ,[TOT_IVA] ,[TOTAL] ,[TOTAL_MN] ,[SALDO_MN] ,[ESTATUS],[ARCHIVOXML]  FROM [DOCVEN]  where TIPO_DOC='FAC' AND NOT (estatus='CAN' OR estatus='PEN') AND cliente = " & id, "dProducto")
    ...
    Private Sub ReturmXML(clave As String)
        Dim DataSet As New DataSet
        Dim id As String = IIf(Session("MM_CLIENTE") Is Nothing, "0", Session("MM_CLIENTE"))
        DataSet = common.sqlconsulta("SELECT [FOLIO],[ARCHIVOXML] FROM [DOCVEN] where TIPO_DOC='FAC' and cliente = " & id & " and clave=" & clave, "dProducto")
        Dim doc As XmlDocument = New XmlDocument()
        Dim foliofac As String = DataSet.Tables(0).Rows(0).Item("FOLIO").ToString
        Dim nombre_archivo As String = RTrim(Session("EMP_RFC")) & RTrim(foliofac) & ".xml"
    ...
    Protected Sub chFill_CheckedChanged(sender As Object, e As EventArgs) Handles chFill.CheckedChanged
        If chFill.Checked = True Then
            ...
            DataSet = common.sqlconsulta("SELECT [CLAVE] ,[EMPRESA] ,[TIPO_DOC] ,[FOLIO] ,[CLIENTE] ,[FECHA_ALTA] ,[FECHA_BAJA] ,[ORDEN_COMP] ,[LISTA] ,[DESC_VOL] ,[SUB_TOTAL] ,[TOT_IVA] ,[TOTAL] ,[TOTAL_MN] ,[SALDO_MN] ,[ESTATUS],[ARCHIVOXML]  FROM [DOCVEN] where TIPO_DOC='FAC' AND NOT (estatus='CAN' OR estatus='PEN') AND SALDO_MN>0 and cliente = " & id, "dProducto")
            ListProducts.DataSource = DataSet
