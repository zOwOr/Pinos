Public Class home
    ...
    Sub BindData()
        ...
        Dim sql As String = "Select top 1000 VW_DOCVEN.tipo_doc,  VW_DOCVEN.folio, VW_DOCVEN.FECHA_ALTA AS fecha, docvende.cantidad,  docvende.unidad , docvende.codigo_pro as IdProduct , producto.descripcion as Producto , docvende.precio_uni , docvende.t_desc_vol, isnull(docvende.tasa_iva,16) as tasa_iva,  docvende.tasa_ieps , docvende.DESCRIP_AM, docvende.cantidad_unidad, docvende.descrip_am From   VW_DOCVEN inner join docvende on VW_DOCVEN.clave = docvende.clave  inner join producto on docvende.codigo_pro = producto.clave where  VW_DOCVEN.TIPO_DOC LIKE 'PED'   AND  VW_DOCVEN.CLIENTE = " & id & "  AND   VW_DOCVEN.empresa = 1 ORDER BY  VW_DOCVEN.CLAVE DESC"
