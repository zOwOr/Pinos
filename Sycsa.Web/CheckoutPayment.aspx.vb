...
Public Class CheckoutPayment
    ...
    Protected Sub btnContinue_Click(sender As Object, e As EventArgs) Handles btnContinue.Click
        ...
        sql = sql & "	SET @FOLIOSIG = 'IN'+  RIGHT('000000' + convert(varchar, (select isnull(max(SUBSTRING(FOLIO,6,6)),0)+1 AS FOLIO from DOCVEN where EMPRESA=1 AND TIPO_DOC='PED' AND SERIE='IN')),6) "
        ...
        For Each _orows As GridViewRow In _ListProducts.Rows
            ...
            sql = sql & " values( @CLAVESIG," & _orows.Cells(0).Text.ToString & ",'PRIM'," & partida & "," & _orows.Cells(6).Text.ToString & "," & _orows.Cells(9).Text.ToString & ",'" & _orows.Cells(4).Text.ToString.ToUpper & "'," & lista & ",round(" & _orows.Cells(5).Text.ToString.ToUpper & ",2),"

            sql = sql & "0,0,'',round(" & _orows.Cells(7).Text.ToString & ",2)," & _orows.Cells(10).Text.ToString & "," & _orows.Cells(11).Text.ToString & ",1,0,0,0,0,'',NULL,NULL,0,0,0"
    ...
    Protected Function GetDescuento() As Decimal
        Dim descuento As Decimal = 0D
        Try
            Dim xmlDoc As New XmlDocument()
            xmlDoc.Load(Server.MapPath("~/local/sql.data.dll.xml"))
            Dim descuentoNode As XmlNode = xmlDoc.SelectSingleNode("/NewDataSet/Conexion/Descuento")
            If descuentoNode IsNot Nothing Then
                descuento = Convert.ToDecimal(descuentoNode.InnerText)
            End If
        Catch ex As Exception
            ' Manejar errores de lectura del archivo XML
        End Try
        Return descuento
    End Function
End Class